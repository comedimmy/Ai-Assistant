document.getElementById("aquarium-form").addEventListener("submit", function(event) {
    ...
    // 發送 POST 請求到後端
    fetch('/api/recommend_aquarium_settings', {
        method: 'POST',
        headers: {
...
window.addEventListener("DOMContentLoaded", () => {
    ...
    const aquariumIdFromURL = urlParams.get("aquarium_id");

    if (aquariumIdFromURL) {
        const aquariumIdInput = document.getElementById("aquarium_id");
        aquariumIdInput.value = aquariumIdFromURL;
        aquariumIdInput.readOnly = true;  // 讓使用者無法更改
    }
});
...
document.getElementById("AI-check-form").addEventListener("submit", function(event) {
    ...
    fetch("/api/activate_aquarium/" + aquariumId, {
        method: "POST",
        headers: {
            'Content-Type': 'application/json',
