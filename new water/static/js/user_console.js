...
if (token) {
    ...
    // 可選：把網址清乾淨，移除 token 參數
    const cleanUrl = window.location.origin + window.location.pathname;
    window.history.replaceState({}, document.title, cleanUrl);
} else {
    console.warn("⚠️ 沒有從 URL 找到 token");
}
fetch("/api/get_aquriums_from_user", {
    method: "POST",
    headers: {
...
.then(data => {
    if (data.aquariums) {
        data.aquariums.forEach(aq => {
            console.log("水族箱ID:", aq.aquarium_id);
            console.log("水族箱名稱:", aq.aquarium_name);
        });
    }
});
